...
(*   Updated: 2026/09/09 00:08:42 by fguarrac         ###   ########.fr       *)
(*                                                                            *)
(* ************************************************************************** *)
...
let complementary_helix (input : helix) : helix =
    let rec comp_helix_acc (original : helix) (output : helix) =
        match original with
        | [] -> output
        | first :: rest -> comp_helix_acc rest (output @ [(Nucleotides.generate_nucleotide (match Nucleotides.get_nucleobase first with
                                           | 'A' -> 'T'
                                           | 'T' -> 'A'
                                           | 'C' -> 'G'
                                           | 'G' -> 'C'
